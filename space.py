...
EXPLORE_DAYS  = {2, 4}   # 수·금: 우주 탐구 (과학·탐사·현상·미션)
...
def _economy_format_rules() -> str:
    return f"""\
[검색 키워드 전략 — 제목·포커스 키워드 선택 기준]
한국 투자자·경제 독자가 실제로 검색하는 키워드 패턴으로 포커스 키워드와 제목을 잡아라.
- 종목·주가 중심: "로켓랩(RKLB) 주가 전망", "한화에어로스페이스 목표주가", "스페이스X 상장 시기"
- 수혜주·정책 중심: "우주항공청 수혜주", "방산 ETF 편입 종목", "누리호 수혜 기업", "K-방산 수출 수혜"
- 서비스·출시 중심: "스타링크 한국 요금", "원웹 국내 서비스", "위성 인터넷 속도 비교"
- 계약·실적 중심: "SpaceX NASA 계약 규모", "한화에어로스페이스 수주 실적", "KAI 방산 수출"
포커스 키워드는 글 주제에서 가장 검색 수요가 높은 위 패턴 하나로 선택한다.
본문에서도 이 키워드 관점을 중심으로 분석한다 — "이 뉴스가 [종목명] 주가·실적에 어떤 의미인지",
"수혜 기업은 누구인지", "한국 투자자가 지금 봐야 할 포인트는 무엇인지".
...
[통찰과 예시 — 뉴스 요약이 아닌 분석글의 핵심]
- 이미 보도된 사실을 그대로 전달하는 글 절대 금지.
  뉴스는 소재일 뿐이다. 글의 핵심은 "이 사실이 왜 중요한지, 내가 어떻게 읽는지"이다.
- 쉬운 비유·예시 최소 1개 필수:
  복잡한 계약 규모·기술 개념은 독자가 즉시 체감할 수 있는 일상적 크기로 비교한다.
  예) "계약액 1조 원 → 한화에어로스페이스 연간 영업이익의 두 배 수준"
      "LEO 위성 고도 550km → 서울-부산 직선 거리의 반도 안 되는 높이"
      "발사 비용이 10분의 1로 줄었다 → 화물기 운임이 갑자기 택배비 수준이 된 것"
      "이 위성 1기 무게 200kg → 성인 남성 3명이 올라선 체중과 같다"
- 내 생각·통찰 최소 1개 문단 필수:
  "이 뉴스에서 제가 주목한 진짜 포인트는 따로 있습니다", "대부분의 기사가 놓친 부분이 있는데요",
  "표면적으로는 X처럼 보이지만, 실제 의미는 Y입니다" 같은 형태로 글쓴이의 해석을 명확히 표현한다.
  단순 사실 나열로 문단을 채우지 말고, 글쓴이가 뉴스를 읽고 내린 판단을 독자와 나눈다.
...
- 문체: 독자에게 친근하게 말하는 경어체로 쓴다. 본문 문장 종결은 반드시 ~습니다/~입니다/~합니다.
  ...
  ❌ 뉴스 기사체 금지: "~것으로 알려졌다", "~에 따르면", "~관계자는" 같은 표현 절대 금지.
  ✅ "이게 왜 중요하냐면요", "쉽게 말씀드리면", "한마디로", "솔직히" 같은 표현 적극 활용.
...
FOCUS_KEYWORD: 투자자·경제 독자가 실제 검색하는 키워드 (예: "로켓랩 주가 전망", "한화에어로스페이스 수혜", "스타링크 한국 요금", "방산 ETF 수혜주")
SEO_TITLE: SEO 제목(60자 이내, 포커스 키워드 앞쪽 — 종목명·주가·수혜주 등 투자 키워드 포함)
SEO_DESCRIPTION: 메타 설명(150~160자, 포커스 키워드 포함, 투자자 클릭 유도)
...
TICKERS: 글 주제와 직접 관련된 상장 종목 1~3개. TradingView 형식으로. 정말 없으면 없음.
  - 글에서 직접 언급한 상장사 우선
...
- 주제 기업이 비상장이더라도 관련 상장 경쟁사·파트너사를 본문에 1~2개 자연스럽게 언급하라.
  예: Rocket Lab(NASDAQ:RKLB), Intuitive Machines(NASDAQ:LUNR), L3Harris(NYSE:LHX) 등

[금지 주제 - 최근 발행됨]
{avoid_str}

[제목 형식 — 투자자가 검색할 키워드 포함]
"로켓랩(RKLB) 주가 전망 — 이번 계약이 만든 변화",
"스타링크 한국 서비스 요금·속도 총정리",
"SpaceX ~억 달러 수주, 수혜 종목은?",
"~가 우주 시장을 바꾸는 방법 — 투자 관점 분석" 등
종목명·주가·수혜주·요금·출시일 같은 실제 검색 키워드가 제목 앞쪽에 오도록.
...
- 본문 내 매수·매도 추천 문구 금지. 산업·시장 분석 중심.
  (※ 글 아래 TICKERS 메타필드 출력은 별개로 반드시 해야 함)

[금지 주제 - 최근 발행됨]
{avoid_str}

[제목 형식 — 투자자가 검색할 키워드 포함]
"한화에어로스페이스(012450) 우주항공청 수혜 분석",
"K-방산 ETF 편입 종목 총정리 — 한화·KAI·LIG넥스원",
"누리호 3차 발사 수혜주는 어디? — KAI·한화 비교",
"한국 방산 수출 ~억 달러, 주가에 미치는 영향" 등
종목명·종목코드·수혜주·ETF 같은 투자 검색 키워드가 제목 앞쪽에 오도록.
...
- 주제 기업이 비상장(SpaceX 등)이더라도, 관련 상장 경쟁사나 파트너사를 본문에서 1~2개 자연스럽게 언급하라.
  예: "SpaceX의 재진입 기술과 경쟁하는 Rocket Lab(NASDAQ:RKLB)", "달 착륙선 납품사 Intuitive Machines(NASDAQ:LUNR)" 등
...
[제목 형식 — 투자자가 검색할 키워드 포함]
"스타링크 한국 서비스 요금·속도 — 국내 통신사 비교",
"위성 인터넷 수혜주 총정리 — KT·SK텔레콤 vs 글로벌 플레이어",
"우주 인터넷 시장 규모와 투자 포인트 — 지금 봐야 할 종목",
"~가 돈이 되는 진짜 이유 — 관련 상장 기업은?" 등
서비스명·요금·속도·수혜주 같은 실제 검색 키워드가 앞에 오도록.
...
[제목 형식 — 투자자가 검색할 키워드 포함]
"로켓랩(RKLB) 주가 전망 — 이번 수주가 의미하는 것",
"보잉 우주사업부 적자, 수혜 종목은 노스롭 그루먼?",
"한화에어로스페이스 주가 — 이번 계약이 실적에 미치는 영향",
"~의 ~억 달러 계약, 지금 주가에 반영됐나?" 등
종목명·주가·실적·수혜 같은 투자 검색 키워드가 앞에 오도록.
...
[제목 형식 — 투자자가 검색할 키워드 포함]
"이번 주 우주 수혜주 변동 — RKLB·BA·012450 흐름 정리",
"우주 산업 이번 주 핵심 — 투자자가 봐야 할 3가지",
"스타링크·방산·달탐사 — 이번 주 우주 경제 투자 포인트" 등
수혜주·종목 티커·투자 포인트 같은 실제 검색 키워드가 포함되도록.
...
# ── 수·금: 우주 탐구 (과학·탐사·현상·미션) ──────────────────
def _post_space_explore(internal_links: str = ""):
    """수요일·금요일 — 우주 탐구 글. 사람들이 검색하는 우주 질문과 최신 발견 중심."""
    print("  🌌 우주 탐구 글 작성 중...")
    avoid_str = _build_avoid_str()
    chat = _build_chat_instruction()

    # 탐구 테마 풀 — 매번 다른 영역에서 주제를 고르도록 다양하게 제시
    theme_pools = [
        "우주 탐사 미션 (Artemis 달 복귀, JWST 최신 발견, Voyager 1·2 근황, 유로파 클리퍼, 화성 탐사 근황)",
        "외계행성 & 외계생명체 (골디락스 존, 외계행성 대기 분석, 페르미 역설, SETI 최신 신호)",
        "블랙홀·중성자별·초신성 (사건 지평선, 중력파 검출, 감마선 버스트, 블랙홀 사진 의미)",
        "태양계 신기한 사실 (유로파·엔켈라두스 내부 바다, 타이탄 메탄 호수, 소행성 채굴, 혜성의 비밀)",
        "별의 일생 & 우주론 (빅뱅 직후 0.001초, 우주 팽창 가속, 다크 에너지·다크 매터, 우주 최후의 날)",
        "우주 물리 & 시간 (시간 팽창, 빛의 속도 한계, 웜홀 가능성, 평행 우주론)",
        "우주 개발 & 인류의 미래 (화성 이주 현실 문제, 우주 방사선, 무중력 신체 변화, 스페이스 콜로니)",
        "최신 천문학 발견 (제임스 웹 망원경 발견, 중력파 새 사건, 새로 발견된 행성·위성·소행성)",
    ]

    intro_styles = [
        '"혹시 이런 거 궁금해본 적 있어?" 라고 독자에게 먼저 질문 던지기',
        '"사실 교과서에는 이렇게 나오는데, 진짜 이야기는 달랐다" 식 반전 오프닝',
        '"이 숫자 들으면 머리가 멍해질 거야" 식 스케일 충격으로 시작',
        '"최근 뉴스에서 봤는데 이게 왜 대단한 건지 설명해줄게" 식 최신 발견 연결',
    ]

    structures = [
        """\
1. 도입부 (3~4문장): 독자가 "어, 나도 이게 궁금했는데!" 할 만한 질문이나 사실로 시작.
2. ## **[기본 개념 — 이게 뭔데?]**: 전공자 아닌 일반인에게 쉽고 재미있게. 비유 필수.
3. ## **[더 파고들면 더 신기한 것]**: 연관 발견, 최신 연구, 의외의 사실.
4. ## **[우리 입장에서 생각해보면]**: 지구·일상·인류 미래와의 연결.
5. 마무리 (2~3문장): "알고 나면 세상이 달라 보인다" 느낌의 여운 있는 마무리.""",

        """\
1. 도입부 (3~4문장): 의외의 사실이나 수치로 충격 후 호기심 유발.
2. ## **[흔한 오해 vs 진짜 과학]**: "대부분 이렇게 알고 있는데 실제론 이렇다".
3. ## **[과학적 원리 — 왜 그런가]**: 구체적 수치·근거·최신 연구. 비유로 체감.
4. ## **[인류가 알아낸 방법]**: 탐사선·망원경·실험이 어떻게 이걸 밝혔는지.
5. 마무리 (2문장): 짧고 인상 깊게.""",

        """\
1. 도입부 (3~4문장): 최신 우주 뉴스나 발견을 후킹 포인트로.
2. ## **[이게 왜 대단한가]**: 발견·현상의 스케일과 의미.
3. ## **[과학 원리 쉽게 풀기]**: 단계적, 비유와 구체적 숫자 필수.
4. ## **[앞으로 어떻게 될까]**: 탐사 계획, 기술 발전, 미래 전망.
5. 마무리 (2~3문장): 우주적 관점의 한 줄 결론.""",
    ]

    prompt = f"""뉴비콘 블로그의 우주 탐구 글을 써라.
이 글은 경제·투자 분석이 아닌, **우주 과학과 탐사에 순수하게 호기심을 느끼는 독자**를 위한 글이다.

[주제 선택 지침]
아래 테마 중 최근 발행되지 않은 영역에서 하나를 골라 구체적인 주제로 좁혀라:
- {chr(10)+'- '.join(theme_pools)}

선택 기준:
1. 최근 실제로 화제가 된 우주 발견·미션이 있으면 그것을 중심으로.
2. "블랙홀에 빨려 들어가면?", "우주는 얼마나 커?", "외계인이 없는 이유는?" 처럼
   사람들이 실제로 검색하는 질문이면 더 좋다.
3. 경제·투자 내용은 완전히 배제. 순수 우주 탐구와 과학 이야기.

[금지 주제 — 최근 발행됨, 의미상 비슷한 것도 피할 것]
{avoid_str}

[제목 형식]
"~하면 어떻게 될까?", "~의 진짜 크기", "~가 사실이었다", "우주에서 ~가 가능할까?",
"제임스 웹이 드디어 발견한 ~", "과학자들이 ~를 발견했다" 등 호기심·경이감 자극 형식.

{build_seo_prompt()}

[형식 조건]
- 분량: 600~800단어. 800단어를 절대 넘지 않는다.
- 대제목(##): 반드시 **굵게**. 2~4개 중 매번 다르게.
- 소제목(###): 각 ## 아래 1~2개 필수.
- 이모지: 대제목·소제목 앞에 맥락에 맞게 (과하지 않게).
- 수치는 반드시 일상 비유로 체감: "1광년" → "빛이 1년 달린 거리 = 자동차로 1억 년"
- 전문용어 첫 등장 시 괄호로 쉬운 설명.
- 리스트(-): 3개 이상 나열 시 필수.
- 굵게(**bold**): 핵심 수치·개념·결론 문장에 1~2개/문단.
- 하이라이트(<mark>): 독자가 반드시 기억할 핵심 1~2곳.
- 문체: 독자에게 친근하게 말하는 경어체. 본문 종결은 반드시 ~습니다/~입니다/~합니다.
  '그렇죠', '~지요', '~네요' 자연스러운 경어체도 허용. ❌ 교과서·기사체 금지.
  ❌ 본문에서 ~다/~이다/~한다 종결 금지 ([CHAT] 블록 안에서만 허용).
  ✅ "솔직히", "이게 진짜 신기한 게요", "쉽게 말씀드리면", "한마디로" 적극 활용.
...
{_meta_output_rules()}"""
    ...
    return _run_and_publish(prompt, mode='우주탐구')
...
- 문체: 독자에게 친근하게 말하는 경어체. 본문 종결은 반드시 ~습니다/~입니다/~합니다.
  '그렇죠', '~지요', '~네요' 자연스러운 경어체도 허용. ❌ 교과서·기사체 금지.
  ❌ 본문에서 ~다/~이다/~한다 종결 금지 ([CHAT] 블록 안에서만 허용).
  ✅ "솔직히", "쉽게 말씀드리면", "이게 진짜 신기한 게요" 적극 활용.
...
- 문체: 독자에게 친근하게 추천하는 경어체. ~습니다/~입니다/~합니다. '그렇죠' 등 허용.
  ❌ 본문에서 ~다/~이다/~한다 종결 금지 ([CHAT] 블록 안에서만 허용).
