...
- 쉬운 비유와 예시를 반드시 활용한다: 어려운 개념이 나오면 독자가 바로 체감할 수 있는
  일상적 상황·크기·금액으로 풀어준다. 최소 1개 필수.
  예) "혈당 스파이크 → 혈당이 롤러코스터처럼 급등락하는 것"
      "체지방 1kg 줄이려면 → 라면 한 박스를 안 먹는 칼로리"
      "하루 30분 걷기 → 한 달이면 서울 외곽 한 바퀴 걷는 거리"
- 내 생각과 통찰을 반드시 넣는다: 정보를 나열하고 끝내지 않는다. "나는 이게 왜 중요하다고 보는지",
  "대부분이 놓치는 포인트가 있다", "직접 해보니 예상과 달랐던 점" 등
  글쓴이의 주관적 해석과 판단을 최소 1개 문단에 담는다.
  이 글을 읽은 독자의 생각이 조금이라도 바뀌어야 한다.
...
[형식 조건]
- 전체 분량: 600~800단어 (한국어 약 2400~3200자). 800단어를 절대 넘지 않는다.
...
- 문체: 독자에게 말하는 경어체로 쓴다. 글 전체에서 ~습니다/~입니다/~합니다/~였습니다로 끝낸다.
  '그렇죠', '~지요', '~네요' 같은 자연스러운 경어체도 허용.
  ❌ 절대 금지: 본문에서 ~다/~이다/~한다/~았다 종결 (그 말투는 [CHAT] 블록 안에서만 허용).
  ✅ 올바른 예: "이게 진짜 핵심입니다.", "직접 해봤더니 달랐습니다.", "모르는 분이 더 많죠."
...
[SEO 최적화 — Rank Math 기준, 글 쓰기 전에 포커스 키워드를 먼저 정한다]
포커스 키워드(2~4단어)를 결정한 뒤, 아래 5곳에 반드시 포함해서 글을 작성한다:
...
def parse_output(raw: str, blog_name: str = "블로그") -> dict:
    ...
    # 경어체 정책 전환으로 normalize_tone 비활성화
...
def build_seo_prompt() -> str:
    """Rank Math 기준 SEO 최적화 지시문 — 전 모듈 공통 사용"""
    return """\
[SEO 최적화 — 반드시 아래 순서대로 진행]

Step 1. 포커스 키워드 결정 (글 작성 전 가장 먼저)
  - 길이: 2~3단어. 짧고 구체적일수록 좋다.
  - 좋은 예: "로켓랩 위성 수주", "편의점 해장 음식", "구독 서비스 절약"
  - 나쁜 예: "우주경제투자분석트렌드" (너무 길), "꿀팁" (너무 막연)

Step 2. 결정한 키워드를 아래 위치에 정확히 그대로 삽입 (변형·동의어 대체 금지)
  ① 제목(# H1): 키워드가 제목 앞쪽(첫 3단어 이내)에 위치
  ② 본문 첫 번째 문장: 키워드가 첫 문장 안에 등장 (50자 이내)
  ③ ## 대제목 최소 1개: 키워드 정확히 포함 (변형어 아닌 동일 표현)
  ④ 본문 전체: 키워드를 정확히 3~5회 반복 (밀도 1~2%, 자연스럽게)
  ⑤ SEO_TITLE: 키워드로 시작 또는 앞쪽 2단어 이내 배치
  ⑥ SEO_DESCRIPTION: 첫 문장에 키워드 포함"""
