...
def _naver_chart_widget(symbol: str) -> str:
    """한국 주식 → 네이버 금융 차트 이미지.
    URL 포맷: area/day/ (area/3month/ 은 404)
    Referer 제한 없음 — 외부 사이트에서 그대로 로드 가능.
    """
    img_url = f"https://ssl.pstatic.net/imgfinance/chart/item/area/day/{symbol}.png"
    ...
    return (
        ...
        f'<img src="{img_url}" alt="{symbol} 주가 차트" loading="lazy" '
